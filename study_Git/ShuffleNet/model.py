...
class InvertedResidual(nn.Module):
    def __init__(self, input_c: int, output_c: int, stride: int):
        ...
        if self.stride == 2:
            self.branch1 = nn.Sequential(
                self.depthwise_conv(input_c, input_c, 3, stride=self.stride, padding=1),
        ...
        self.branch2 = nn.Sequential(
            ...
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
            self.depthwise_conv(branch_features, branch_features, 3, stride=self.stride, padding=1),
            nn.BatchNorm2d(branch_features),
    ...
    @staticmethod
    def depthwise_conv(input_c: int, output_c: int, kernel_s: int, stride: int, padding: int,
                       bias: bool = False) -> nn.Conv2d:
        return nn.Conv2d(input_c, output_c, kernel_s, stride, padding, bias=bias, groups=input_c)
...
class ShuffleNetV2(nn.Module):
    def __init__(self, stages_repeats: List[int],
        ...
        for name, repeat, output_channels in zip(stage_names, stages_repeats, self._stage_out_channels[1:]):
            # 每个stage的首个单元需要下采样且stride为2
