...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, downsample=None, groups=1, width_per_group=64, **kwargs):
...
class ResNet(nn.Module):
    def __init__(self, block, blocks_num, num_classes=1000, include_top=False, groups=1, width_per_group=64, **kwargs):
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        if stride != 1 or self.in_channel != channel * block.expansion:
            downs_sample = nn.Sequential(
                nn.Conv2d(self.in_channel, channel * block.expansion, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(channel * block.expansion),
            )

        layers = []
        layers.append(block(self.in_channel, channel, stride=stride, downsample=downs_sample, groups=self.groups,
